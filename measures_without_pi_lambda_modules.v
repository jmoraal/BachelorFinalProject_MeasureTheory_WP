...
Definition full_set_in_set {U} (Λ : setOfSubsets U) 
  : Prop :=
    Ω (*Full_set U*) ∈ Λ. 

Definition complement_in_set {U} (Λ : setOfSubsets U) 
  : Prop := 
    ∀ A  : subset U, A ∈ Λ 
      ⇒ (Ω \ A) ∈ Λ. 

Definition closed_under_disjoint_countable_union {U} (Λ : setOfSubsets U) 
  : Prop :=
    ∀ C : (ℕ → (subset U)), 
      (∀ m n : ℕ, m ≠ n ⇒ Disjoint _ (C m) (C n)) 
        ⇒ (∀ n : ℕ, (C n) ∈ Λ) ⇒  (Countable_union C) ∈ Λ.

Definition closed_under_countable_union {U} (Λ : setOfSubsets U) 
  : Prop :=  
    ∀ C : (ℕ → (subset U)), (∀ n : ℕ, (C n) ∈ Λ) 
      ⇒ (Countable_union C) ∈ Λ.

Definition is_λ_system {U} (Λ : setOfSubsets U) 
  : Prop :=
    full_set_in_set Λ ∧ 
    complement_in_set Λ ∧
...
Definition is_σ_algebra {U} (F : setOfSubsets U) 
  : Prop := 
    full_set_in_set F ∧ 
    complement_in_set F ∧
...
Definition  σ_algebra_generated_by {U} (A : setOfSubsets U) 
  : (setOfSubsets U) := 
...
Definition restriction {U} (F : setOfSubsets U) (A : (subset U)) 
  : (setOfSubsets U) := 
...
Check ∅.
(*
Record emptyset {V : Type} := make_emptyset { 
  und_set :  subset V; proof_is_empty : und_set = @empty V}.
Check und_set.
Coercion und_set : emptyset >-> Ensemble.

Definition test {V : Type} (A : emptyset) := Empty_set V.
Hint Resolve (∅ = Empty_set U) : sets.
*)
