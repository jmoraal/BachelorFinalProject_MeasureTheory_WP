...
Definition subset U := (Ensemble U). 

Definition setOfSubsets U := (Ensemble (subset U)). 
...
(*
Definition in_salg F A := 
  In _ (underlying_set_of_subsets_sigma F) A.
Check in_salg.
Definition salg_class := (σ_algebra ⇨ subset U).
Definition my_in (F : setOfSubsets U) : (subset U -> Prop) := 
  fun (A : subset U) => (In _ F A).
Check my_in.
Definition my_class := setOfSubsets U ⇨ subset U.
