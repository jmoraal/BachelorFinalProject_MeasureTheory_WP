...
Ltac TakeTest x y := 
  match y with 
    | "an_increasing_sequence_of_sets"%string => Take x : (@subset U)
    | "an_increasing_sequence_of_sets"%string 
      => Take C : (ℕ → (@subset U)); Assume C_is_increasing : is_increasing_seq_sets C
  end.

Tactic Notation "Let" ident(x) "be" "a" ident(y) := 
  intros x;
  Assume y.

Tactic Notation "Let" ident(A) "be" ident(x) := 
  match x with 
    | "a_@subset" => Take A : (@subset U)
    | "a_set_of_@subsets" => Take A : (@setOfSubsets U)
  end.
*)
...
Definition Countable_union_on {V : Type} (A : (ℕ → @subset V) ) 
  : @subset V := 
    ｛ x : V | ∃n : ℕ, x ∈ (A n)｝.

Definition full_set_in_set_on {V : Type} (Λ : @setOfSubsets V) 
  : Prop :=
    Full_set V ∈ Λ. 

Definition complement_in_set_on {V : Type} (Λ : @setOfSubsets V) 
  : Prop := 
    ∀A  : @subset V, A ∈ Λ 
      ⇒ (Full_set V \ A) ∈ Λ. 

Definition closed_under_countable_union_on {V : Type} (Λ : @setOfSubsets V) 
  : Prop :=  
    ∀ C : (ℕ → (@subset V)), (∀ n : ℕ, (C n) ∈ Λ) 
      ⇒ (Countable_union_on C) ∈ Λ.

Definition is_σ_algebra_on {V : Type} (F : @setOfSubsets V) 
  : Prop := 
    full_set_in_set_on F ∧ 
    complement_in_set_on F ∧
    closed_under_countable_union_on F.
...
Record > σ_algebras : Type
  := {σ_algebra :> @setOfSubsets U; 
      σ_cond : is_σ_algebra σ_algebra}.
...
Record sigma_algebra {V : Type} := make_sigma_algebra
  { underlying_set_of_subsets : @setOfSubsets V;
    proof_is_sigma_algebra : is_σ_algebra_on underlying_set_of_subsets}.
...
(*
Definition is_underlying F := underlying_set_of_subsets G = F.
Hint Rewrite (is_underlying) : measure_theory.
*)
(* SECOND WAY 
Definition my_universe := Type.
Definition my_sig {V : my_universe} {P : my_universe -> Prop } := sig P.

Definition new_sigma_algebra { V : Type } :=
  @sig (Ensemble (Ensemble V)) (@is_σ_algebra_on V).

Definition my_coercion {V : Type} : 
  @new_sigma_algebra V -> (@setOfSubsets V) :=
  (@proj1_sig (Ensemble (Ensemble V)) (is_σ_algebra_on)).

Coercion my_coercion : new_sigma_algebra >-> setOfSubsets.

Variable H : @new_sigma_algebra U.

Definition F : setOfSubsets := H.
*)

Check (is_σ_algebra_on F). 
...
Definition σ_additive_on F (μ : (subset ⇨ ℝ)) : Prop := 
  ∀C : (ℕ → (subset)), (∀n : ℕ, C n ∈ F) 
    ⇒ (∀ m n : ℕ, m ≠ n ⇒ Disjoint _ (C m) (C n)) 
...
Definition is_measure_on F (μ : (subset → ℝ)) : Prop := 
  μ ∅ = 0 ∧ μ is_σ-additive_on F.

Definition is_probability_measure_on F (μ : (subset → ℝ)) 
  : Prop := 
...
By classic it holds that 
  (∀ n, aux_prop n ∨ ¬aux_prop n) (aux_prop_decidable). 
By dec_inh_nat_@subset_has_unique_least_element it holds that
  (has_unique_least_element le aux_prop) (exists_least_n). 
Choose n1 such that x_in_C_minimal_n according to exists_least_n. 
