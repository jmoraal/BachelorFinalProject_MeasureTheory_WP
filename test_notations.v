...
(* Introducing existing variable, via Ltac: *)
Ltac TakeTest x y := 
  match y with 
    | "a_subset_of_U" => Take x : (subset U)
    | "an_increasing_sequence_of_sets" 
      => Take x : (ℕ → (subset U))(*; Assume C_is_increasing : (is_increasing_seq_sets C)*)
  end.

Tactic Notation "TestLet" ident(x) "be" ident(y) := 
  TakeTest x y.

(* Same principle, but directly in a notation: *)
Tactic Notation "We" "take" ident(x) ident(A)  := 
  match x with 
    | "a subset" => Take A : (subset U)
    | "a set of subsets" => Take A : (setOfSubsets U)
    | "an increasing sequence of sets" 
      => Take x : (ℕ → (subset U)); Assume is_increasing_seq_sets
  end.

(* Condition on a variable: *)
Notation "x 'is' 'a' y" := 
  match y with 
    | σ_algebra => is_σ_algebra x 
    | "a π-system" => is_π_system x
    | "a λ-system" => is_λ_system x 
(*    | "an increasing sequence of sets" => is_increasing_seq_sets x *)
    | _ => False 
  end (at level 50).
...
Lemma test_intersection_empty : ∀ A : subset U, 
  (A ∩ ∅) = ∅. 
...
Proof. 
(*TestLet C be an_increasing_sequence_of_sets.*)
We take "an increasing sequence of sets" C. 
Take C : (ℕ ⇨ subset U). 
